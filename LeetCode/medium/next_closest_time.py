...
class Solution:
    def nextClosestTime(self, time: str) -> str:
        ...
        # check if there is a value in the same hour
        i = two_digit_values.index(minute)
        if len(two_digit_values) > i+1 and two_digit_values[i+1] < "60":
            return hour + ":" + two_digit_values[i+1]
        
        # check if there is value in the same day
        i = two_digit_values.index(hour)
        if len(two_digit_values) > i+1 and two_digit_values[i+1] < "24":
            return two_digit_values[i+1]  + ":" + two_digit_values[0]
        
        # check the first value next day
